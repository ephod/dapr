...
class SavingsAccountData(TypedDict):
    balance: int
    savings_account: str
    customer: str
    uuid: str
...
class SavingsAccountActor(Actor, SavingsAccountActorInterface, Remindable):
    def __init__(self, ctx, actor_id):
        super(SavingsAccountActor, self).__init__(ctx, actor_id)
    ...
    async def set_up(self, data: SavingsAccountData) -> None:
        """An actor method to set up class."""
        print(f"SavingsAccountActor.set_up", flush=True)
        data["uuid"] = str(uuid4())
        await self._state_manager.set_state("savings_account_actor", data)
        await self._state_manager.save_state()

    async def deposit(self, amount: int) -> None:
        """Deposit request."""
        print(f"SavingsAccountActor.deposit", flush=True)
        val: SavingsAccountData
        has_value, val = await self._state_manager.try_get_state(
            "savings_account_actor"
        )
        val["balance"] += amount
        print(
    ...
    async def show_balance(self) -> int:
        ...
        print(f"SavingsAccountActor.show_balance", flush=True)
        val: SavingsAccountData
        has_value, val = await self._state_manager.try_get_state(
            "savings_account_actor"
        )
    ...
    async def withdrawal(self, amount: int) -> int:
        """Withdrawal request."""
        print(f"SavingsAccountActor.withdrawal", flush=True)
        val: SavingsAccountData
        has_value, val = await self._state_manager.try_get_state(
            "savings_account_actor"
        )
        if val["balance"] >= amount:
            ...
            await self._state_manager.set_state("savings_account_actor", val)
            await self._state_manager.save_state()
            return amount
        else:
            print(
